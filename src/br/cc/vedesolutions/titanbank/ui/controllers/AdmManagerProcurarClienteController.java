...
import br.cc.vedesolutions.titanbank.model.utils.FormatMask;
import br.cc.vedesolutions.titanbank.model.utils.MessageDialog;
import br.cc.vedesolutions.titanbank.model.utils.Relatorio;
	...
	void ContexMenuOnAction(ActionEvent event) throws FileNotFoundException {
		...
		if (menu.getText().toLowerCase().contains("relatório")) {
			...
			if (tbViewClientes.getSelectionModel().getSelectedItems().size() > 0) {
				...
				MessageDialog message = new MessageDialog("TitanBank", null, "Usuário(s) removido(s) com sucesso.",
						AlertType.INFORMATION);
				message.showDialog();
			} else {
				MessageDialog message = new MessageDialog("TitanBank", null, "É preciso selecionar pelo menos um item.",
						AlertType.INFORMATION);
				message.showDialog();
			}
	...
	public boolean ShowClienteDialog(Cliente cliente) {
		try {
			FXMLLoader loader = new FXMLLoader(getClass().getClassLoader()
					.getResource("./br/cc/vedesolutions/titanbank/ui/views/AdmUserInforEditScreen.fxml"));
