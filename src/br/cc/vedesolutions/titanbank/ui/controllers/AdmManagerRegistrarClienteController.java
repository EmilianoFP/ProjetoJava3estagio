...
import br.cc.vedesolutions.titanbank.model.exception.ContaJaCadastradaException;
    ...
    void btnSalvarOnAction(ActionEvent event) throws ContaJaCadastradaException, SQLException, IOException {
    	...
    	try {
    		...
    		if (clienteDAO.adicionar(cliente) == true){
    			...
    			message.setDialogMessage("Houve algum erro no cadastro do cliente, verifique os parametros.");
    			message.showDialog();
    		}
    		...
    		message.setDialogMessage("Por algum motivo nao foi possivel adicionar o cliente no banco de dados.");
    		message.showDialog();
    		System.out.println("Debug: BtnSalvarOnAction@AdmManagerRegistrarCliente: "+e.getMessage());
    	} finally {
    		cliente = null;
